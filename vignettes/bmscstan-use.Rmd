...
- Body District : Congruency $BF_{10} = 3.81$, $\mu = -10.03$,
  $95\%~CI = [-19.84; -0.375]$.
...
```{r}
pp <- pairwise.BMSC(mdl , contrast = "Body.District2:Congruency2" ,
                    who = "control")
...
- Body District : Congruency $BF_{10} = 8.47$, $\mu = -16.82$,
  $95\%~CI = [-31.99; -1.750]$.
- Body District : Side $BF_{10} = 7.31$, $\mu = -16.61$,
  $95\%~CI = [-31.49; -1.474]$.
- Body District : Congruency : Side $BF_{10} = 6.37$, $\mu = -16.85$,
  $95\%~CI = [-33.74; -11.10]$.
...
Model comparison can be done by means of the `loo_compare.loo_BMSC`
function:
...
package and in the @Vehtari2016 and @Vehtari2015.
...
data.sim$y <- y
data.sim$bin <- sapply(invlogit(data.sim$y), function(x) rbinom( 1, 1, x))
...
data.pt$bin <- sapply(invlogit(data.pt$y), function(x) rbinom( 1, 1, x))
...
```{r}
mdlBinH1 <- BMSC(formula = cbind(bin, n) ~ 1 + Cond,
            data_ctrl = data.sim.bin, data_sc = data.pt.bin, seed = 25,
            typeprior = "normal", family = "binomial", s = 1, cores = 1)

mdlBinH0 <- BMSC(formula = cbind(bin, n) ~ 1,
            data_ctrl = data.sim.bin, data_sc = data.pt.bin, seed = 52,
            typeprior = "student", family = "binomial", s = 1, cores = 1)
```

Finally, to compare the two models, we will use the
Leaving-One-Out criterion.

```{r}
looBinH1 <- loo.BMSC( mdlBinH1 )
looBinH0 <- loo.BMSC( mdlBinH0 )

loo_compare.loo_BMSC( looBinH1, looBinH0 )
```
