...
test_that("bmsc_stan produces expected errors", {
  ...
  expect_error(BMSC(y~x*cond+(cond|id), data_ctrl = dat.ctrl),
               "the dataframe \"data_pt\" is not specified")
  expect_error(BMSC(y~x*cond+(cond|id), data_pt = dat.pt),
               "the dataframe \"data_ctrl\" is not specified")
  expect_error(BMSC(y~x*cond+(cond|id), data_ctrl = dat.ctrl, data_pt = dat.pt, typeprior = "gamma"),
